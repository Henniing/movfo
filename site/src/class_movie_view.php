...
class movie_view {
    ...
    public function __construct($movie) {
        $this->movie = $movie;
        $this->movie_view = $this->generate_movie_view($movie->title, $movie->rating, $movie->votes, $movie->genres, $movie->year, $movie->country, $movie->languages, $movie->youtube_embedd_link[0], $movie->torrentlist);
    }
    ...
    private function generate_movie_view($title, $rating, $votes, $genres, $year, $country, $languages, $youtube_embedd_link, $torrentlist){
        $out = $this->generate_sidebar($title, $rating, $votes, $genres, $year, $country, $languages, $youtube_embedd_link);
        $out .= $this->generate_torrentlist($torrentlist);
        return $out;
    }

    private function generate_sidebar($title, $rating, $votes, $genres, $year, $country, $languages, $youtube_embedd_link){
        $out = "<div id='sidebar'>";
        $out .= $this->generate_info($title, $rating, $votes, $genres, $year, $country, $languages);
    ...
    private function generate_info($title, $rating, $votes, $genres, $year, $country, $languages){
        $rating_grphx = "";
        $r = (string)$rating;
        $r1 = substr($r, 0, 1);
        $r2 = substr($r, 2, 1);
